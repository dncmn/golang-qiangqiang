...
function sendAjaxRequet() {
    ...
    if (partUrl == "toFindPowerList") {//权限列表
        ...
        $.post(url, data, function (res) {
            ...
            $.each(tmp.PowerInfo, function (n, value) {
    ...
    if (partUrl == "toFindPowerByName") { //权限查询
...
function requestUserPower(nickName, userId, trId, btnId) {
    ...
    console.log(timeStr + "   " + nickName + "    " + userId + "    " + trId + "   " + btnId);

    var isPower = confirm("是否给玩家授权");
    //1、给玩家授权
    if (isPower) {
...
function requestCanclePower(trId, calBtnId, nickName, userId) {
    ...
    var timeStr = getTimeToMinuteStr() + ":" + new Date().getSeconds();
    console.log(timeStr + "   " + nickName + "    " + userId + "    " + trId + "   " + calBtnId);
...
function addForm2(index, nickArr, idsArr, OpArr, OpTimeNumArr, StatusArr) {
    var timeStr = getTimeToMinuteStr() + ":" + new Date().getSeconds();
    console.log("addForm2" + "   " + timeStr + "   " + nickArr + "    " + idsArr);
...
function confirmUrl() {
    ...
    urlArray[0] = "toFindPowerList"; //权限列表
    urlArray[1] = "toFindPowerByName";//根据你玩家昵称或者玩家id查找玩家信息
